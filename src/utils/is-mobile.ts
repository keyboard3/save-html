...
const isAppleTabletOnIos13 = (navigator?: Navigator): boolean => {
  return (
    ...
    // @ts-ignore: it's ok
    navigator.maxTouchPoints > 1 && typeof globalThis.MSStream === "undefined"
  );
};
...
export default function isMobile(param?: IsMobileParameter): isMobileResult {
  ...
  result.phone = result.apple.phone || result.android.phone ||
    result.windows.phone;
  result.tablet = result.apple.tablet || result.android.tablet ||
